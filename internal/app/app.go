...
func (app *App) Run() {
	...
	utils.PrintLabelValue("📱", "支持平台", "蝴蝶号")
	...
	// 启动下载后自动处理服务（抽音频 + 转文字），默认启用
	if app.Cfg.AutoTranscribe {
		_ = services.GetPostProcessService() // 触发单例初始化 + worker 启动
	} else {
		utils.Info("自动转录未启用 (auto_transcribe: false)")
	}
	...
	go func() {
		...
		if os_env == "windows" {
			...
			if ok := app.Sunny.StartProcess(); ok {
				utils.Info("✓ 蝴蝶号注入引擎已就绪 (WeChatAppEx.exe)")
			} else {
				utils.Warn("⚠️ 注入引擎启动失败：可能需要 [管理员权限] 才能在蝴蝶号内显示按钮")
			}
		}
	...
	services.GetPostProcessService().Stop()
}
...
func (app *App) printTitle() {
	...
	color.Yellow("    蝴蝶号下载助手 v%s", app.Cfg.Version)
	color.Yellow("    项目地址：https://github.com/huige-opc/wx-video-channel")

...
type Config struct {
	...
	// 自动后处理：视频下载完成后自动抽音频 + 转文字
	AutoTranscribe    bool   `mapstructure:"auto_transcribe"`     // 是否启用（默认 true）
	BaiduASRKey       string `mapstructure:"baidu_asr_key"`       // 百度语音识别 API_KEY（可选，为空时用 transcribe.py 内置）
	BaiduASRSecret    string `mapstructure:"baidu_asr_secret"`    // 百度语音识别 SECRET_KEY
	PythonExe         string `mapstructure:"python_exe"`          // Python 可执行文件路径（默认 python）
	PostProcessWorker int    `mapstructure:"post_process_worker"` // 后处理并发 worker 数（默认 2）

	// AI 逐字稿清洗模型（OpenAI 兼容接口）
	LLMApiKey   string `mapstructure:"llm_api_key"`    // 大模型 API Key
	LLMApiBase  string `mapstructure:"llm_api_base"`   // API 地址，默认 https://api.openai.com/v1
	LLMModel    string `mapstructure:"llm_model"`       // 模型名，默认 gpt-4o-mini
}
...
func setDefaults() {
	...
	// 自动后处理默认值
	viper.SetDefault("auto_transcribe", true) // 默认开启：下载完自动抽音频+转文字
	viper.SetDefault("baidu_asr_key", "")     // 为空则用 transcribe.py 里的内置 Key
	viper.SetDefault("baidu_asr_secret", "")
	viper.SetDefault("python_exe", "python")
	viper.SetDefault("post_process_worker", 2)

	// AI 清洗默认值
	viper.SetDefault("llm_api_key", "")
	viper.SetDefault("llm_api_base", "https://api.openai.com/v1")

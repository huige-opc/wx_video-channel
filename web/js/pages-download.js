...
async function startQueueItemDownload(id) {
    ...
    if (!item.decryptKey) {
        showMessage('缺少解密密钥，请从蝴蝶号页面使用批量下载功能', 'warning');
        console.warn('下载队列项缺少解密密钥。蝴蝶号视频是加密的，需要从浏览器页面获取解密密钥。');
        return;
    }
...
function updateSharedFeedBackendHint() {
    ...
    if (batchSharedFeedStatus.backendEnabled) {
        ...
        hint.textContent = `后端解析可用（${label}）。自动模式会优先走纯后端，失败后回退到蝴蝶号页面。`;
        return;
    }

    hint.textContent = '后端解析未配置。自动模式将仅在后端不可用时尝试蝴蝶号页面；如需纯后端，请在 config.yaml 配置 cloudflare.sphCookie 或 cloudflare.sphHostname。';
}

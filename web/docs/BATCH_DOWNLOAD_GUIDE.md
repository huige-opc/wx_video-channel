...
批量下载功能允许一次性下载多个蝴蝶号视频，支持普通视频和加密视频的自动处理。
...
- 自动重试（可配置次数，默认3次）
- 指数退避 + 随机抖动（2s, 4s, 8s... + 0-1s随机）
- Context 超时控制（可配置，默认30分钟）
- 详细的重试日志
- 支持断点续传（非加密视频）
...
### 下载性能
- 并发：可配置并发数（默认5，通过 DownloadConcurrency 配置）
- 重试：可配置重试次数（默认3次，通过 DownloadRetryCount 配置）
- 超时：可配置超时时间（默认30分钟，通过 DownloadTimeout 配置）
- 缓冲：32KB 缓冲区
- 断点续传：支持（非加密视频，通过 DownloadResumeEnabled 配置）
...
### Q: 下载速度慢？
A: 默认并发数为5，可以在配置中调整 DownloadConcurrency。注意并发过高可能导致服务器限流。
...
### Q: 可以并发下载吗？
A: 支持！默认并发数为5，可通过配置 DownloadConcurrency 调整。

...
```bash
# 在编译时注入版本号和构建时间
go build -ldflags="-s -w -X main.Version=1.0.0 -X main.BuildTime=$(date +%Y%m%d%H%M%S)" -o 蝴蝶号下载助手.exe
```
      ...
      "0000": {
        ...
        "info": {
          "0409": {
            ...
            "FileDescription": "一个用于下载蝴蝶号内容的工具",
            "FileVersion": "1.0.0.0",
            "ProductName": "蝴蝶号下载助手",
...
```bash
# 1. 清理旧文件
rm -f 蝴蝶号下载助手.exe wx_video-channel_cloud.exe
...
- `蝴蝶号下载助手.exe`：普通版，构建时 `cloud_enabled=false`、`radar_enabled=false`
- `wx_video-channel_cloud.exe`：Hub 版，构建时 `cloud_enabled=true`、`radar_enabled=false`
- `wx_video-channel_radar.exe`：雷达版，构建时 `cloud_enabled=false`、`radar_enabled=true`
...
- `release/v<version>/wx_video-channel_v<version>.exe`
- `release/v<version>/wx_video-channel_v<version>.zip`
- `wx_video-channel_cloud.exe`
- `蝴蝶号下载助手.exe`
- `wx_video-channel_radar.exe`
- `release/v<version>/wx_video-channel_cloud_v<version>.exe`
- `release/v<version>/wx_video-channel_cloud_v<version>.zip`
- `release/v<version>/wx_video-channel_radar_v<version>.exe`
- `release/v<version>/wx_video-channel_radar_v<version>.zip`
...
# 2. 创建发布目录
mkdir -p release/wx_video-channel_v1.0.0

# 3. 复制必要文件
cp 蝴蝶号下载助手.exe release/wx_video-channel_v1.0.0/
cp README.md release/wx_video-channel_v1.0.0/
cp -r docs release/wx_video-channel_v1.0.0/

# 4. 创建 ZIP 压缩包
cd release
zip -r wx_video-channel_v1.0.0.zip wx_video-channel_v1.0.0/

# 或使用 7-Zip（Windows）
# 7z a wx_video-channel_v1.0.0.zip wx_video-channel_v1.0.0/
```
...
echo [1/4] 清理旧文件...
if exist 蝴蝶号下载助手.exe del 蝴蝶号下载助手.exe
...
GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o 蝴蝶号下载助手.exe
if [ $? -ne 0 ]; then
    echo "错误: 编译失败"
    exit 1
fi
...
```bash
# 确保设置了正确的环境变量
GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o 蝴蝶号下载助手.exe

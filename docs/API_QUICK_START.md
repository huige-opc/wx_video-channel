...
本项目提供了完整的 HTTP API 接口，可以通过标准的 HTTP 请求获取蝴蝶号的数据。
...
1. **必须先打开蝴蝶号页面** - API 通过 WebSocket 与页面通信
2. **使用正确的 username** - 从搜索结果中获取，不是昵称
3. **请求频率** - 建议在请求之间添加适当延迟（0.5-1秒）
4. **错误处理** - 检查 `code`，0 表示成功
5. **对标雷达默认关闭** - 雷达监控需要运行 `wx_video-channel_radar.exe`，且默认以 `config.yaml` 中的 `radar_enabled` 为准
...
- 普通版 `蝴蝶号下载助手.exe` 默认不启用对标雷达
- 需要使用 `wx_video-channel_radar.exe`

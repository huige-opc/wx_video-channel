...
Web 控制台是基于浏览器的现代化图形界面，提供完整的蝴蝶号视频管理功能。
...
- 并发下载数 - 同时下载任务数 (1-5)
- 文件名附带视频 ID - 控制默认命名是否追加 `video_id`
- 下载文件名模板 - 只读展示 `config.yaml` 中的 `download_filename_template`
...
**配置持有项说明**：
- `download_filename_template` 需要在 `config.yaml` 中修改，控制台仅展示当前值和支持的占位符。
- `radar_enabled` 需要在 `config.yaml` 中修改并重启程序，控制台仅展示当前状态。

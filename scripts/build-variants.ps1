...
function New-ZipPackage {
    ...
    $tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("wx_video-channel_pkg_" + [System.Guid]::NewGuid().ToString('N'))
...
$variants = @(
    [pscustomobject]@{
        Label = 'standard'
        RootExe = '蝴蝶号下载助手.exe'
        ReleaseName = "wx_video-channel_v$version.exe"
        ReleaseZip = "wx_video-channel_v$version.zip"
        CloudEnabled = $false
        RadarEnabled = $false
        PackageExeName = '蝴蝶号下载助手.exe'
    },
    [pscustomobject]@{
        Label = 'cloud'
        RootExe = 'wx_video-channel_cloud.exe'
        ReleaseName = "wx_video-channel_cloud_v$version.exe"
        ReleaseZip = "wx_video-channel_cloud_v$version.zip"
        CloudEnabled = $true
        RadarEnabled = $false
        PackageExeName = 'wx_video-channel_cloud.exe'
    },
    [pscustomobject]@{
        Label = 'radar'
        RootExe = 'wx_video-channel_radar.exe'
        ReleaseName = "wx_video-channel_radar_v$version.exe"
        ReleaseZip = "wx_video-channel_radar_v$version.zip"
        CloudEnabled = $false
        RadarEnabled = $true
        PackageExeName = 'wx_video-channel_radar.exe'
    }
)
...
try {
    ...
    foreach ($staleItem in @(
        "wx_video-channel_v$version.exe",
        "wx_video-channel_v$version.zip",
        "wx_video-channel_cloud_v$version.exe",
        "wx_video-channel_cloud_v$version.zip",
        "wx_video-channel_radar_v$version.exe",
        "wx_video-channel_radar_v$version.zip"
    )) {
